...
class CarAgencyManager {
  ...
  transferCarBetweenAgencies(fromAgencyId, toAgencyId, carId) {
    ...
    const car = this.findCar(fromAgencyId, carId);
    if (fromAgency && toAgency && car) {
      const uCar = {
        brand: car.brand.brand,
        models: [car.brand.models[car.brandArrIndex]],
      };
      this.addCarToAgency(toAgencyId, uCar);
      [car.brand.models[car.brandArrIndex]][0].ownerId = toAgencyId;
  ...
  searchCustomer(idOrName) {
    if (typeof idOrName === "string") {
      for (let customer of this.customers) {
        if (customer.name === idOrName || customer.id === idOrName) {
          return customer;
        }
      }
      return undefined;
...
class CarManager {
  ...
  searchCars(year, price, brand) {
    ...
    if (price) {
      cars = cars.filter((car) => car.price === price);
    }
    if (brand) {
  ...
  getMostExpensiveCar() {
    ...
    let mostExpensiveCar = allCars[0];
...
class CarPurchaseManager {
  ...
  sellCar(carId, customerId) {
    ...
    let isCustomerHasEnoughMoney = false;
    let isCarExist = false;
    for (let agency of this.agencies) {
      // console.log(agency.agencyId);
      const car = carAgencyManager.findCar(agency.agencyId, carId);
      // console.log(car);
      if (car) {
        isCarExist = true;
        const uCar = car.brand.models[car.brandArrIndex];
        // console.log(uCar);
        const customer = customerManager.searchCustomer(customerId);
        // console.log(customer);
        if (customer) {
          const carPriceIncludingVat = uCar.price + uCar.price * 0.17;
          if (customer.cash >= carPriceIncludingVat) {
            isCustomerHasEnoughMoney = true;
            // console.log(customer);
            customer.cash -= carPriceIncludingVat;
            // console.log(customer);
            // console.log(agency);
            agency.cash += uCar.price;
            // console.log(agency);
            // console.log(car.brand.models);
            carAgencyManager.removeCarFromAgency(agency.agencyId, carId);
            // console.log(car.brand.models);
            uCar.ownerId = customerId;
            // console.log(customer.cars);
            customer.cars.push(uCar);
            // console.log(customer.cars);

            this.taxesAuthority.totalTaxesPaid += uCar.price * 0.17;
            this.taxesAuthority.sumOfAllTransactions += carPriceIncludingVat;
            this.taxesAuthority.numberOfTransactions += 1;
          }
        }
      }
    }
    if (isCarExist && isCustomerHasEnoughMoney) {
      return true;
    } else if (!isCarExist) {
      console.log("The vehicle does not exist at the agency!");
    } else if (!isCustomerHasEnoughMoney) {
  ...
  getTotalMarketRevenue() {
    return this.taxesAuthority.sumOfAllTransactions;
  }
}
...
// console.log("Search Cars (No Brand): ", carManager.searchCars(2010, 129000));
...
// // Test sellCar
// console.log("Sell Car: ", carPurchaseManager.sellCar("KqKV_", "BGzHhjnE8"));
// console.log(
...
//   carPurchaseManager.sellCar("S6DL1", "Wm6BkA1F0")
// );
